/**
 * Every API route, defined once against an Express-shaped `app`.
 *
 * This file was the body of the old Express server. It is unchanged apart from
 * taking `app` and `db` as arguments instead of closing over module-level ones,
 * so the handlers are the same code that ran on the server — the platform just
 * runs them in the browser now. See ./router.js for the shim that provides
 * `app`, and ./index.js for the fetch-shaped entry point the UI calls.
 *
 * Handlers are synchronous and mutate `db` in place: the POC's create/approve
 * workflows are real, they simply live for as long as the tab does.
 */
import * as ai from './ai.js';
import * as kpi from './kpi.js';
import * as content from './content.js';
import { REFERENCE_DATE } from './clock.js';
import { TABLES } from './tables.js';

const BOOTED_AT = Date.now();

export function defineRoutes(app, db) {
